 ...
 *
 * Study port of `79588b42cb`. Observation / test harness only: production tick callers are not
 * touched. PRODUCTION / UNFLOORED read exported coefficients, so the fixture stays a before/after
 * reference after the P0.11 floor-as-sensitivity rewrite.
 */
object BarrierReplay {
    ...
    private val PCT = NumberFormat.DECIMAL_1
    private val UNITS_3 = NumberFormat.DECIMAL_3
    ...
    data class Reproduction(
        ...
        fun format(): String = buildString {
            ...
            appendLine("  gamma branch reproduced      $gammaReproduced (${pct(gammaRate)} %)")
            appendLine("  safe_u reproduced            $safeUReproduced (${pct(safeURate)} %)")
            append("  permitted total reproduced   $permittedReproduced (${pct(permittedRate)} %)")
        }
    }
    ...
    data class Comparison(
        ...
        private fun line(result: ScenarioResult): String {
            val units = UNITS_3.format(result.totalPermittedU, NumberFormatPlatform.SEPARATOR_DOT)
            return "  ${padEnd(result.label, 26)} ${padStart(result.ticks.toString(), 5)}   ${padStart(units, 11)}   ${padStart(result.fullSuspensions.toString(), 16)}"
        }
    }
    ...
    private fun pct(value: Double): String = PCT.format(value, NumberFormatPlatform.SEPARATOR_DOT)

    private fun padEnd(value: String, width: Int): String =
        if (value.length >= width) value else value + " ".repeat(width - value.length)

    private fun padStart(value: String, width: Int): String =
        if (value.length >= width) value else " ".repeat(width - value.length) + value
     ...
     *
     * Kept local so `commonTest` does not grow a mockito / shared-tests dependency.
     */
