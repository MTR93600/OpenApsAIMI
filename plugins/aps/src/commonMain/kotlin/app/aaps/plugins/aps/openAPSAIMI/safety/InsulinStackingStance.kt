...
object InsulinStackingStance {
     ...
     *
     * WARNING: `MealAbsorptionPhaseEngine` has a copy of the OLD formula on purpose. There the same
     * number means the opposite thing — it is a condition for the PEAK_CORRECTION phase, which
     * SWITCHES OFF this surveillance — so lowering it there would remove protection instead of
     * adding it. The two must not be changed together again.
     */
    ...
    fun evaluate(
        ...
        val summary = buildString {
            ...
            append("iob=${aimiFmt2(iobSafe)}U floor=${aimiFmt2(iobFloor)} | ")
            append("Δ=${aimiFmt1(delta)} sΔ=${aimiFmt1(shortAvgDelta)} plateau_ok | ")
            if (ev != null) append("ev=${aimiFmt0(ev)} ")
            if (mn != null) append("minPred=${aimiFmt0(mn)} ")
            if (trajectoryEnergy != null) append("trajE=${aimiFmt2(trajectoryEnergy)} ")
