...
data class LoopProfile(
    ...
    val lgsThreshold: Double,
    val minBg: Double? = null
)
...
/**
 * Exhaustive handling of a verdict. The KMP engine does not enact here.
 * The Android/iOS shell applies Applied commands after evaluate().
 */
fun processDecision(result: DecisionResult) {
    when (result) {
        is DecisionResult.Applied -> Unit
        is DecisionResult.Rejected -> Unit
        is DecisionResult.Skipped -> Unit
        is DecisionResult.Fallthrough -> Unit
    }
}
