...
data class TrajectoryAnalysis(
    ...
    fun toConsoleLog(): List<String> {
        val fmt3 = NumberFormat(minFractionDigits = 3)
        fun f3(value: Double): String = fmt3.format(value, NumberFormatPlatform.SEPARATOR_DOT)
        fun signedF3(value: Double): String {
            val body = fmt3.format(abs(value), NumberFormatPlatform.SEPARATOR_DOT)
            return if (value >= 0.0) "+$body" else "-$body"
        }
        fun signedF2(value: Double): String {
        ...
        log.add("  Metrics:")
        log.add("    Curvature: ${f3(metrics.curvature)} ${if (metrics.isTightSpiral) "⚠️ HIGH" else ""}")
        log.add("    Convergence: ${signedF3(metrics.convergenceVelocity)} mg/dL/min ${
            when {
        ...
        log.add("    Coherence: ${aimiFmt2(metrics.coherence)} ${if (metrics.hasLowCoherence) "⚠️ LOW" else ""}")
        log.add("    Energy: ${signedF2(metrics.energyBalance)}U ${if (metrics.energyBalance > 2.5) "⚠️" else ""}")
        log.add("    Openness: ${aimiFmt2(metrics.openness)} ${if (metrics.openness > 0.7) "⚠️ WIDE" else ""}")
        log.add("    Health: ${aimiFmt1(metrics.healthScore * 100)}%")
        
        if (modulation.isSignificant()) {
            log.add("  Modulation:")
            log.add("    SMB damping: ${aimiFmt2(modulation.smbDamping)}x")
            log.add("    Interval: ${aimiFmt2(modulation.intervalStretch)}x")
            log.add("    Basal pref: ${aimiFmt0(modulation.basalPreference * 100)}%")
            log.add("    Safety margin: ${aimiFmt2(modulation.safetyMarginExpand)}x")
