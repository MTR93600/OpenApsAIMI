...
data class OrefAnalysisReport(
    ...
    fun toPromptSection(): String = buildString {
        ...
        timeInRange70180Pct?.let { append("CGM TIR 70-180 (merged timeline): ${aimiFmt1(it)}%\n") }
        timeBelow70Pct?.let { append("CGM TBR <70: ${aimiFmt1(it)}%\n") }
        timeAbove180Pct?.let { append("CGM TAR >180: ${aimiFmt1(it)}%\n") }
        actualHypo4hPct?.let { append("4h hypo exposure (labelled windows): ${aimiFmt1(it)}%\n") }
        actualHyper4hPct?.let { append("4h hyper exposure (labelled windows): ${aimiFmt1(it)}%\n") }
        ...
        if (meanRawHypoRiskPct != null || meanCalHypoRiskPct != null) {
            meanRawHypoRiskPct?.let { append("ONNX hypo risk (raw mean): ${aimiFmt1(it)}%\n") }
            meanCalHypoRiskPct?.let { append("ONNX hypo risk (calibrated mean): ${aimiFmt1(it)}%\n") }
        }
        if (meanRawHyperRiskPct != null || meanCalHyperRiskPct != null) {
            meanRawHyperRiskPct?.let { append("ONNX hyper risk (raw mean): ${aimiFmt1(it)}%\n") }
            meanCalHyperRiskPct?.let { append("ONNX hyper risk (calibrated mean): ${aimiFmt1(it)}%\n") }
        }
        meanBgChangePred?.let { append("ONNX mean predicted BG change: ${aimiFmt2(it)}\n") }
        ...
        personalMeanHypoSignalPct?.let { append("Personal hypo score (uncalibrated internal number, NOT a probability, do not report as a risk): ${aimiFmt1(it)}\n") }
        personalMeanHyperSignalPct?.let { append("Personal hyper score (uncalibrated internal number, NOT a probability, do not report as a risk): ${aimiFmt1(it)}\n") }
        ...
        if (worstMissing.isNotEmpty()) {
            ...
            append(worstMissing.joinToString { "${it.key}=${aimiFmt0(it.value)}%" })
            append("\n")
        }
    ...
    fun toCoachUserInsightsSection(): String = buildString {
        ...
        timeInRange70180Pct?.let { append("Time in range 70–180 mg/dL: ${aimiFmt0(it)}%.\n") }
        timeBelow70Pct?.let { append("Time below 70 mg/dL: ${aimiFmt0(it)}%.\n") }
        timeAbove180Pct?.let { append("Time above 180 mg/dL: ${aimiFmt0(it)}%.\n") }
