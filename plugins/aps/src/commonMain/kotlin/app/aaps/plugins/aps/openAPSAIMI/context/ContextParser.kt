...
class ContextParser @Inject constructor(
    ...
    fun parse(text: String): List<ContextIntent> {
        ...
        aapsLogger.debug(LTag.AIMI, "[ContextParser] Parsing offline: '$text'")
        ...
        if (intents.isEmpty()) {
            aapsLogger.warn(LTag.AIMI, "[ContextParser] No pattern matched: '$text'")
        } else {
            aapsLogger.info(LTag.AIMI, "[ContextParser] Parsed ${intents.size} intent(s) offline")
        }
    ...
    private fun extractDuration(text: String): Duration? {
