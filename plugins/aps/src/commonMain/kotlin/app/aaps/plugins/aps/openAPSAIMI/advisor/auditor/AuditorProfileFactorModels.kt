...
data class AuditorProfileProposal(
     ...
     */
    fun toJsonLine(parentEventId: String): String = JsonObj().apply {
        ...
        put(
            ...
            JsonObj().apply {
                ...
                put(
                    "claims",
                    JsonObj().apply {
                        put("bg_start_mgdl", llm.claims.bgStartMgdl ?: AimiJson.NULL)
                        put("bg_end_mgdl", llm.claims.bgEndMgdl ?: AimiJson.NULL)
                        put("bg_min_mgdl", llm.claims.bgMinMgdl ?: AimiJson.NULL)
                        put("iob_start_u", llm.claims.iobStartU ?: AimiJson.NULL)
                        put("iob_end_u", llm.claims.iobEndU ?: AimiJson.NULL)
                        put("insulin_delivered_u", llm.claims.insulinDeliveredU ?: AimiJson.NULL)
                    },
                )
                put("parse_issues", JsonArr(llm.parseIssues))
                put("failure", llm.failure ?: AimiJson.NULL)
            },
        )
        put(
            "validated",
            JsonObj().apply {
                ...
                put("refused_by", JsonArr(refusedBy))
            },
        )
        put(
            "claim_check",
            JsonArr().apply {
                claimCheck.forEach { row ->
                    put(
                        JsonObj().apply {
                            put("key", row.key)
                            put("llm", row.llmValue ?: AimiJson.NULL)
                            put("data", row.dataValue ?: AimiJson.NULL)
                            put("ok", row.ok)
                        },
                    )
                }
        ...
        put(
            "context",
            runCatching {
                val element = Json.parseToJsonElement(contextJson)
...
class AuditorProfileTickState {
     ...
     */
    fun toJsonObject(): JsonObject = JsonObj().apply {
        val held = proposal
        put("schema", AuditorProfileFactorLimits.AUDIT_SCHEMA_V1)
        put("key_on", keyOn)
        put("status", status)
        put("audit_id", held?.auditId ?: AimiJson.NULL)
        put("context_built_at_ms", held?.contextBuiltAtMs ?: AimiJson.NULL)
        put("verdict_age_ms", isfDecision?.ageMs ?: targetDecision?.ageMs ?: AimiJson.NULL)
        ...
        put("llm_confidence", held?.confidence ?: AimiJson.NULL)
        put("arrival_refused_by", JsonArr(held?.refusedBy ?: emptyList<String>()))
        ...
        put(
            ...
            JsonObj().apply {
                ...
                put("path", isfPath)
                put("refused_by", JsonArr(isfDecision?.refusedBy ?: emptyList<String>()))
        ...
        put(
            ...
            JsonObj().apply {
                ...
                put("refused_by", JsonArr(targetDecision?.refusedBy ?: emptyList<String>()))
                put("path", targetPath)
                put("dose_sites", JsonArr(targetDoseSites.toList()))
    ...
    private fun TickSafety?.toJsonOrNull(): Any {
        val safety = this ?: return AimiJson.NULL
     ...
     */
    private fun JsonObj.putNumberOrNull(name: String, value: Double?) {
        put(name, value?.takeIf { it.isFinite() } ?: AimiJson.NULL)
    }
}
