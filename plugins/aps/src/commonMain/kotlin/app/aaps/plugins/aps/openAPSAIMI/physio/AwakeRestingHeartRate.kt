 ...
 *
 * Source: `origin/dev_OAPSAIMI` @ `6a6561caab`. Study: commonMain. Local time uses
 * [kotlinx.datetime.TimeZone] instead of `java.time.ZoneId`. Window, centile, sample floor and
 * awake hours are the numbers of that commit: 7 days, 10th centile, 50 samples on 3 distinct days,
 * local hours [AWAKE_FIRST_HOUR] until [NIGHT_FIRST_HOUR].
 */
object AwakeRestingHeartRate {
     ...
     */
    fun estimate(samples: List<Sample>, zoneId: TimeZone): Int? {
    ...
    private fun isUsable(sample: Sample, zoneId: TimeZone): Boolean {
        ...
        val hour = Instant.fromEpochMilliseconds(sample.timestampMs).toLocalDateTime(zoneId).hour
