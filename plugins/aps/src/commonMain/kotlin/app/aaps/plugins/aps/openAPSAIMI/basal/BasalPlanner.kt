    ...
    fun plan(ctx: LoopContext): BasalPlan? {
        ...
        if (mgdl <= HYPO_SUSPEND_MGDL) {
            if (d5 < 0.0) {
                return BasalPlan(
                    rateUph = 0.0,
                    durationMin = HYPO_SUSPEND_MIN,
                    reason = "Soft Hypo guard: BG=$mgdl, Δ=${aimiFmt1(d5)} < 0 -> suspend"
                )
            } else {
                ...
                return BasalPlan(
                    rateUph = rate,
                    durationMin = HYPO_SUSPEND_MIN,
                    reason = "Soft Hypo guard (rising): BG=$mgdl, Δ=${aimiFmt1(d5)} >= 0 -> safe basal ${aimiFmt2(rate)}U/h"
                )
            }
        }
        ...
        if (d5 < -2.0 && projected30 < lgs) {
            return BasalPlan(
                rateUph = 0.0,
                durationMin = HYPO_SUSPEND_MIN,
                reason = "Predictive Low: Bg ${mgdl.toInt()} -> ${projected30.toInt()} < $lgs (ShortΔ ${aimiFmt1(short)})"
            )
        }
        ...
        if (lastTempIsZero && zeroSinceMin >= ZERO_RESUME_MIN) {
            ...
            return BasalPlan(
                rateUph = rate,
                durationMin = dur,
                reason = "Micro-resume after ${zeroSinceMin}m @0U/h → ${aimiFmt2(rate)}U/h × ${dur}m"
            )
        }
        ...
        if (mgdl > HIGH_BG && abs(d5) < PLATEAU_DELTA_ABS && ctx.iobU < 1.5) {
            ...
            if (!isRising) {
                ...
                return BasalPlan(
                    ...
                    durationMin = 20,
                    reason = "High-flat kicker (BG=${mgdl.toInt()}, Delta=${aimiFmt2(d5)})"
                )
            }
        }
        ...
        if (nearFlat && d5 < DELTA_POS_RELEASE) {
            ...
            return BasalPlan(
                rateUph = rate,
                durationMin = minDur,
                reason = "Anti-stall (Δ≈0) → ${aimiFmt2(rate)}U/h × ${minDur}m"
            )
        }
