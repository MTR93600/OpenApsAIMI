 ...
 *
 * Pure data, no I/O, so it lives in commonMain: [AimiAppendGuard][app.aaps.plugins.aps.openAPSAIMI.utils.AimiAppendGuard]
 * (the commonMain seam) and the Android-only janitor (`AimiRetentionManager`) both read this same
 * table, on their own platform, so a cap and a retention rule for the same file can never disagree.
 */
