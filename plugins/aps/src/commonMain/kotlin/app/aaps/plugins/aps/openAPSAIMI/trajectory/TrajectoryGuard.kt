...
class TrajectoryGuard @Inject constructor(
    ...
    fun analyzeTrajectory(
        ...
        if (history.size < MIN_HISTORY_STATES) {
            aapsLogger.debug(LTag.AIMI, "TrajectoryGuard: Insufficient history (${history.size} states, need $MIN_HISTORY_STATES)")
            return null
        }
        ...
        aapsLogger.debug(
            LTag.AIMI,
            "TrajectoryGuard: ${classification.emoji()} ${classification.name} " +
                "κ=${fmt3(metrics.curvature)} v_conv=${aimiFmt2(metrics.convergenceVelocity)} " +
                "ρ=${aimiFmt2(metrics.coherence)} E=${aimiFmt2(metrics.energyBalance)} Θ=${aimiFmt2(metrics.openness)}"
        )
        
        if (modulation.isSignificant()) {
            aapsLogger.info(LTag.AIMI, "TrajectoryGuard: Modulation active - ${modulation.reason}")
        }
        
        warnings.forEach { warning ->
            when (warning.severity) {
                WarningSeverity.CRITICAL -> aapsLogger.error(LTag.AIMI, "TrajectoryGuard CRITICAL: ${warning.message}")
                WarningSeverity.HIGH -> aapsLogger.warn(LTag.AIMI, "TrajectoryGuard: ${warning.message}")
                WarningSeverity.MEDIUM -> aapsLogger.info(LTag.AIMI, "TrajectoryGuard: ${warning.message}")
                WarningSeverity.LOW -> aapsLogger.debug(LTag.AIMI, "TrajectoryGuard: ${warning.message}")
            }
        }
        ...
        return when (classification) {
            ...
            TrajectoryType.OPEN_DIVERGING -> {
                ...
                TrajectoryModulation(
                    ...
                    reason = "Diverging, need stronger action (coherence=${aimiFmt2(metrics.coherence)})"
                )
            }
            ...
            TrajectoryType.TIGHT_SPIRAL -> {
                ...
                TrajectoryModulation(
                    ...
                    reason = "Trajectory compressed - over-correction risk (E=${aimiFmt2(metrics.energyBalance)}U, κ=${fmt3(metrics.curvature)})"
                )
            }
            ...
            TrajectoryType.CLOSING_CONVERGING -> {
                ...
                TrajectoryModulation(
                    ...
                    reason = "Closing naturally (v=${signedFmt2(metrics.convergenceVelocity)})"
                )
            }
    ...
    private fun generateWarnings(
        ...
        if (metrics.energyBalance > ENERGY_STACKING && metrics.curvature > 0.2) {
            ...
            warnings.add(TrajectoryWarning(
                ...
                message = "Multiple corrections accumulating (E=${aimiFmt2(metrics.energyBalance)}U) - hypo risk in 60-90 min",
        ...
        if (metrics.coherence < COHERENCE_LOW && history.last().iob > 2.0) {
            warnings.add(TrajectoryWarning(
                ...
                type = "LOW_COHERENCE",
                message = "IOB ${aimiFmt2(history.last().iob)}U present but BG not responding (ρ=${aimiFmt2(metrics.coherence)})",
        ...
        if (metrics.openness > 0.75 && 
            ...
            warnings.add(TrajectoryWarning(
                ...
                message = "BG drifting upward (${history.last().bg.toInt()} mg/dL) despite IOB - trajectory not closing (Θ=${aimiFmt2(metrics.openness)})",
        ...
        if (history.last().pkpdStage == ActivityStage.RISING &&
            ...
            warnings.add(TrajectoryWarning(
                ...
                message = "Fresh IOB (${aimiFmt2(history.last().iob)}U) in PRE_ONSET but trajectory already tightening (κ=${fmt3(metrics.curvature)})",
        ...
        if (metrics.coherence < -0.3 && history.last().insulinActivity > 1.5) {
            warnings.add(TrajectoryWarning(
                ...
                message = "BG rising despite high insulin activity (${aimiFmt2(history.last().insulinActivity)} U/hr) - negative coherence (ρ=${aimiFmt2(metrics.coherence)})",
        ...
        if (classification == TrajectoryType.STABLE_ORBIT && metrics.healthScore > 0.85) {
            warnings.add(TrajectoryWarning(
                ...
                message = "Excellent glycemic control - stable orbit maintained (health=${aimiFmt0(metrics.healthScore * 100)}%)",
    ...
    private fun fmt3(value: Double): String =
        NumberFormat(minFractionDigits = 3).format(value, NumberFormatPlatform.SEPARATOR_DOT)

    private fun signedFmt2(value: Double): String {
