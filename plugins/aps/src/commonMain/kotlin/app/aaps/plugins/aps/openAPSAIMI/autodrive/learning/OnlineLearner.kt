...
class OnlineLearner @Inject constructor(
    ...
    private var evaluatedFeedbackCount = 0L
    ...
    fun learnAndUpdate(currentState: AutoDriveState, currentEpochMs: Long) {
        lock.withLock {
            ...
            if (currentState.bg > 140.0 && currentState.bgVelocity > 0.8 && learnedSensitivityFactor > 1.0) {
                 ...
                 learnedSensitivityFactor = max(1.0, learnedSensitivityFactor - releaseStep)
                 releaseCount += 1
                 aapsLogger.info(LTag.AIMI, "🎓 [ONLINE_LEARNING] 🏃 Exercise Release Triggered! BG=${currentState.bg.toInt()} Vel=${aimiFmt2(currentState.bgVelocity)} -> Normalizing Sensitivity to ${learnedSensitivityFactor.format(3)}")
            }
            ...
            val matchedEntry = predictionHistory.entries.find { 
                abs(it.key - currentEpochMs) < toleranceMs 
            }
            ...
            val staleKeys = predictionHistory.keys.filter { it < currentEpochMs - (60 * 60 * 1000) }
            staleKeys.forEach { predictionHistory.remove(it) }
            publishStatus(currentEpochMs)
        }
    }

    fun statusSnapshot(): StatusSnapshot = lock.withLock { snapshot }
    ...
    private fun Double.format(digits: Int) =
        NumberFormat.withDecimals(digits).format(this.toDouble(), NumberFormatPlatform.SEPARATOR_DOT)
}
