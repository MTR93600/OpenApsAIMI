 ...
 *
 * Source: `origin/dev_OAPSAIMI` @ `e87cb03e87` + `9d0e9bdb9f`, awake-baseline reason from `6a6561caab`.
 * Study wiring: commonMain shared `object` — pure, no clock, no Android. KMP adaptation only:
 * [aimiFmt1] for one-decimal reason fragments. [REASON_NO_BASELINE] and the missing-data split match
 * `6a6561caab`.
 */
    ...
    fun evaluate(
        ...
        if (!signatureHolds) {
            ...
            if (wasActive && !continuityBroken && !releases) {
                val brokenText = "broken=${aimiFmt1(brokenMinutes)}min"
        ...
        val heldText = "held=${aimiFmt1(heldMinutes)}min"
        return Verdict(
            active = active,
