...
class AIMIPhysioFeatureExtractorMTR @Inject constructor(
    ...
    fun extractFeatures(
        ...
        if (!rawData.hasAnyData()) {
            aapsLogger.debug(LTag.AIMI, "[$TAG] No data available - returning empty features")
    ...
    private fun extractHRVFeatures(
        ...
        val targetSamples = if (sleep != null && sleep.hasValidData()) {
            ...
            if (nocturnal.isNotEmpty()) {
                //aapsLogger.debug(LTag.AIMI, "[$TAG] Using ${nocturnal.size} nocturnal HRV samples (Sleep overlap)")
                nocturnal
            } else {
                recentSamples
            }
    ...
    private fun logFeatures(features: PhysioFeaturesMTR) {
        if (!features.hasValidData) {
            aapsLogger.debug(LTag.AIMI, "[$TAG] No valid features extracted")
            return
        }
        ...
        aapsLogger.info(
            ...
            "Sleep: ${aimiFmt1(features.sleepDurationHours)}h (eff=${(features.sleepEfficiency * 100).toInt()}%), " +
            "SleepQ: ${(features.sleepQualityScore * 100).toInt()}%, " +
            "HRV: ${aimiFmt1(features.hrvMeanRMSSD)}ms, " +
