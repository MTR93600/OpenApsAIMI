            ...
            when (type) {
                "Activity" -> {
                    ...
                    ContextIntent.Activity(
                        ...
                        intensity = ContextIntent.Intensity.valueOf(obj.getValue("int").jsonPrimitive.content),
                        confidence = (obj.getValue("conf").jsonPrimitive.doubleOrNull ?: error("JSON field is not a double.")).toFloat(),
                        activityType = ContextIntent.Activity.ActivityType.valueOf(obj.getValue("act").jsonPrimitive.content)
                    )
                }
                ...
                "Stress" -> {
                    ...
                    ContextIntent.Stress(
                        ...
                        intensity = ContextIntent.Intensity.valueOf(obj.getValue("int").jsonPrimitive.content),
                        confidence = (obj.getValue("conf").jsonPrimitive.doubleOrNull ?: error("JSON field is not a double.")).toFloat(),
                        stressType = ContextIntent.Stress.StressType.valueOf(obj.getValue("stress").jsonPrimitive.content)
                    )
                }
                
                "Illness" -> {
                    val start = obj.optLongCompat("start", 0L)
                    ContextIntent.Illness(
                        startTimeMs = if (start > 0) start else aimiWallClockMs(),
                        durationMs = obj.getValue("dur").jsonPrimitive.longOrNull ?: error("JSONObject[\"dur\"] not a long."),
                        intensity = ContextIntent.Intensity.valueOf(obj.getValue("int").jsonPrimitive.content),
                        confidence = (obj.getValue("conf").jsonPrimitive.doubleOrNull ?: error("JSON field is not a double.")).toFloat(),
                        symptomType = ContextIntent.Illness.SymptomType.valueOf(obj.getValue("symptom").jsonPrimitive.content)
                    )
                }
                
                "UnannouncedMeal", "UnannouncedMealRisk" -> {
                    val start = obj.optLongCompat("start", 0L)
                    ContextIntent.UnannouncedMealRisk(
                        startTimeMs = if (start > 0) start else aimiWallClockMs(),
                        durationMs = obj.getValue("dur").jsonPrimitive.longOrNull ?: error("JSONObject[\"dur\"] not a long."),
                ...
                "SlowCarbMeal", "SlowMeal", "FatMeal" -> {
                    val start = obj.optLongCompat("start", 0L)
                    ContextIntent.SlowCarbMeal(
                        startTimeMs = if (start > 0) start else aimiWallClockMs(),
                        durationMs = obj.getValue("dur").jsonPrimitive.longOrNull ?: error("JSONObject[\"dur\"] not a long."),
                        intensity = runCatching { ContextIntent.Intensity.valueOf(obj.let { o -> if (o.hasCompat("int")) o.optStringCompat("int") else "MEDIUM" }) }.getOrDefault(ContextIntent.Intensity.MEDIUM),
                        confidence = (obj.getValue("conf").jsonPrimitive.doubleOrNull ?: error("JSON field is not a double.")).toFloat()
                    )
                }
                ...
                "MenstrualCycle" -> {
                    val start = obj.optLongCompat("start", 0L)
                    ContextIntent.MenstrualCycle(
                        startTimeMs = if (start > 0) start else aimiWallClockMs(),
                        durationMs = obj.getValue("dur").jsonPrimitive.longOrNull ?: error("JSONObject[\"dur\"] not a long."),
                        intensity = ContextIntent.Intensity.valueOf(obj.getValue("int").jsonPrimitive.content),
                        confidence = (obj.getValue("conf").jsonPrimitive.doubleOrNull ?: error("JSON field is not a double.")).toFloat(),
                        phase = ContextIntent.MenstrualCycle.CyclePhase.valueOf(obj.getValue("phase").jsonPrimitive.content)
                    )
                }
                
                "Custom" -> {
                    val start = obj.optLongCompat("start", 0L)
                    ContextIntent.Custom(
                        startTimeMs = if (start > 0) start else aimiWallClockMs(),
                        durationMs = obj.getValue("dur").jsonPrimitive.longOrNull ?: error("JSONObject[\"dur\"] not a long."),
                        intensity = ContextIntent.Intensity.valueOf(obj.getValue("int").jsonPrimitive.content),
                        confidence = (obj.getValue("conf").jsonPrimitive.doubleOrNull ?: error("JSON field is not a double.")).toFloat(),
                    ...
                    aapsLogger.warn(LTag.AIMI, "[ContextDeserializer] Unknown type: $type")
                    null
                }
            }
        } catch (e: Exception) {
            aapsLogger.error(LTag.AIMI, "[ContextDeserializer] Parse failed: ${e.message}", e)
            null
        }
    }
}
