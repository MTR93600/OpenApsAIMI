 ...
 * 
 * IMPACT ON SAFETY:
 * By filtering at 0.5, we ensure that the [app.aaps.plugins.aps.openAPSAIMI.trajectory.TrajectoryGuard] only acts on 
    ...
    fun compute(input: GateInput): PhysioModulation {
        ...
        if (input.dataQuality < minQuality) {
            return PhysioModulation.NEUTRAL.copy(
                ...
                debug = "Low Quality ${aimiFmt2(input.dataQuality)} < $minQuality"
            )
        }
        ...
        val debugStr = buildString {
            append("DQ=${aimiFmt2(input.dataQuality)} ")
            append("Vec=[${vector.joinToString(",") { aimiFmt1(it) }}] ")
            append("W=[${weights.entries.joinToString(" ") { 
                "${it.key.name.take(1)}:${aimiFmt2(it.value)}" 
            }}] ")
            append("-> M=${aimiFmt2(clampedSens)} S=${clampedShift}m")
        }
    ...
    private fun calculateWeights(
        ...
        return if (sumExps == 0.0) {
            kernels.associate { it.name to (if (it.name == KernelType.REST) 1.0 else 0.0) }
             ...
             val result = mutableMapOf<KernelType, Double>()
