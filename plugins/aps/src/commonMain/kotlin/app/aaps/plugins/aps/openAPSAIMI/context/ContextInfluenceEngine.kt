...
class ContextInfluenceEngine @Inject constructor(
    ...
    fun computeInfluence(
        ...
        aapsLogger.debug(LTag.AIMI, "[ContextInfluence] Computing for ${snapshot.intentCount} intent(s)")
        ...
        aapsLogger.info(LTag.AIMI, "[ContextInfluence] Result: smb=$clampedSmb interval=+${clampedInterval}min preferBasal=$preferBasal")
...
mode: ContextMode,
    ...
    private fun processSlowCarbMeal(
        ...
        reasoning.add(
            "SlowCarbMeal ${maxIntensity.name} ${if (earlyPhase) "EARLY" else "ABSORBING"} " +
                "→ SMB×${modeSmb.format(2)} ceiling=${guardedCeiling?.let { "${aimiFmt1(it)}U" } ?: "none"} +${intervalAdd}min"
        )
    ...
    private fun Float.format(decimals: Int): String {
        return NumberFormat.withDecimals(decimals).format(this.toDouble(), NumberFormatPlatform.SEPARATOR_DOT)
    }
