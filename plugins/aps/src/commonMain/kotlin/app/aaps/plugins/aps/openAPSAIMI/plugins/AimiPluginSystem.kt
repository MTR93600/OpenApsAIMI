...
class AimiPluginManager @Inject constructor(
    ...
    fun register(plugin: AimiDecisionPlugin) {
        if (plugins.any { it.id == plugin.id }) {
            aapsLogger.warn(LTag.AIMI, "Plugin ${plugin.id} already registered. Skipping.")
            return
        }
        plugins.add(plugin)
        plugins.sortByDescending { it.priority }
        aapsLogger.info(LTag.AIMI, "🧩 AimiPluginManager: Registered ${plugin.name} (Priority: ${plugin.priority})")
    }
        ...
        plugins.forEach { plugin ->
            try {
                ...
                if (proposed.isNotEmpty()) {
                    aapsLogger.debug(LTag.AIMI, "🧩 Plugin ${plugin.id} proposed ${proposed.size} actions")
                ...
                aapsLogger.error(LTag.AIMI, "🧩 Plugin ${plugin.id} failed during analysis", e)
            }
        }
