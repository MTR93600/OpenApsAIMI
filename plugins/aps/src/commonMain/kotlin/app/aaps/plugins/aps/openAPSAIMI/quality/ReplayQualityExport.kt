...
internal object ReplayQualityExportBuilder {
    ...
    fun toJsonObject(export: ReplayQualityExport): JsonObject =
        buildJsonObject {
            ...
            putOpt("meal_hypothesis_confidence", export.mealHypothesisConfidence)
            put("uam_hypothesis_dominant", export.uamHypothesisDominant)
            putOpt("uam_hypothesis_dominant_confidence", export.uamHypothesisDominantConfidence)
            ...
            putOpt("patient_mode", export.patientMode)
            putOpt("patient_mode_confidence", export.patientModeConfidence)
            putOpt("patient_strategy_hint", export.patientStrategyHint)
            put("patient_mode_reasons", JsonArray(export.patientModeReasons.map { JsonPrimitive(it) }))
            putOpt("causal_state_dominant", export.causalStateDominant)
            putOpt("causal_state_confidence", export.causalStateConfidence)
            putOpt("causal_meal_confidence", export.causalMealConfidence)
            putOpt("causal_protective_confidence", export.causalProtectiveConfidence)
            putOpt("causal_learning_quality", export.causalLearningQuality)
            put("context_intent_active", export.contextIntentActive)
            putOpt("context_intent_count", export.contextIntentCount)
            putOpt("context_intent_dominant", export.contextIntentDominant)
            putOpt("pattern_dominant", export.patternDominant)
            ...
            putOpt("correction_aggression_tier", export.correctionAggressionTier)
            ...
            putOpt("authority_readiness_score", export.authorityReadinessScore)
            put("authority_gate_reasons", JsonArray(export.authorityGateReasons.map { JsonPrimitive(it) }))
            putOpt("load_governor_tier", export.loadGovernorTier)
            put("prediction_available", export.predictionAvailable)
            putOpt("smb_proposed_u", export.smbProposedU)
            putOpt("smb_capped_u", export.smbCappedU)
            putOpt("smb_final_u", export.smbFinalU)
            put("decision_source", export.decisionSource)
            put("safety_source", export.safetySource)
            put("quality_tags", JsonArray(export.qualityTags.map { JsonPrimitive(it) }))
            put("tuning_reference", export.tuningReference)
        }

    private fun JsonObjectBuilder.putOpt(key: String, value: String?) {
        if (value != null) put(key, value) else put(key, JsonNull)
    }

    private fun JsonObjectBuilder.putOpt(key: String, value: Double?) {
        if (value != null) put(key, value) else put(key, JsonNull)
    }

    private fun JsonObjectBuilder.putOpt(key: String, value: Int?) {
        if (value != null) put(key, value) else put(key, JsonNull)
    }
