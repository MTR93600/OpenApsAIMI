...
class TrajectoryHistoryProvider @Inject constructor(
    ...
    suspend fun buildHistory(
        ...
        try {
            ...
            if (filteredBgReadings.isEmpty()) {
                aapsLogger.warn(LTag.AIMI, "TrajectoryHistory: No BG readings found in last $historyMinutes min")
                ...
                if (bg.timestamp >= lastSampledTime + sampleInterval ||
                    ...
                    val iobTotalObj = if (effectiveProfile != null) {
                        try {
                            ...
                            aapsLogger.error(LTag.AIMI, "Error getting IOB at t=${bg.timestamp}: ${e.message}")
                            null
                        }
                    } else {
                        try {
                            ...
                            aapsLogger.error(LTag.AIMI, "Error getting IOB for history: ${e.message}")
                            null
                        }
                    }
            ...
            aapsLogger.error(LTag.AIMI, "Error building trajectory history: ${e.message}")
            return@withContext listOf(
                createCurrentState(
                    nowMillis, currentBg, currentDelta, currentAccel,
                    insulinActivityNow, iobNow, pkpdStage, timeSinceLastBolus, cobNow
        ...
        aapsLogger.debug(LTag.AIMI, "TrajectoryHistory: Built ${history.size} states over $historyMinutes min")
        history
    }
    ...
    private suspend fun estimateTimeSinceLastBolus(timestamp: Long): Int {
        try {
            ...
            aapsLogger.debug(LTag.AIMI, "Could not estimate time since last bolus: ${e.message}")
        }
