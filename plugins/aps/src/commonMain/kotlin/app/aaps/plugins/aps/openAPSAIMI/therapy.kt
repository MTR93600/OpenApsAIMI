...
class Therapy(private val persistenceLayer: PersistenceLayer) {
    ...
    fun updateStatesBasedOnTherapyEvents(forceRefresh: Boolean = false) {
        ...
        lock.withLock { snapshot }?.let { applySnapshot(it) }
        refreshIfNeededAsync()
    }

    private fun refreshBlocking() {
        runCatching {
            runBlocking(aapsIoDispatcher) { buildSnapshot() }
        }.onSuccess { snap ->
            lock.withLock { snapshot = snap }
            applySnapshot(snap)
        }.onFailure {
            lock.withLock { snapshot }?.let { applySnapshot(it) } ?: resetAllStates()
        }
    }
    ...
    private suspend fun buildSnapshot(): TherapySnapshot {
        val now = aimiWallClockMs()
        val fromTime = now - T.days(1).msecs()
    ...
    private fun isCalibrationEvent(events: List<TE>, now: Long): Boolean {
        val cutoff = now - T.mins(15).msecs()
    ...
    private fun extractDateFromDeleteEvent(note: String?): String? {
        val deletePattern = Regex("delete (\\d{2}/\\d{2}/\\d{4})", RegexOption.IGNORE_CASE)
        val match = deletePattern.find(note ?: "")
    ...
    fun getTimeElapsedSinceLastEvent(keyword: String): Long {
        val now = aimiWallClockMs()
        val fromTime = now - T.mins(60).msecs()
