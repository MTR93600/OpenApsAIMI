...
class DynIsfTrajectoryTuning @Inject constructor(
    ...
    fun computeAdjustedIsf(
        ...
        val logDetail =
            "DYNISF_TRAJ rise=${aimiFmt2(riseScore)} fall=${aimiFmt2(fallScore)} " +
                "mult=${NumberFormat(minFractionDigits = 4).format(mult, NumberFormatPlatform.SEPARATOR_DOT)} blended=${aimiFmt1(blendedMgdlPerU)} -> " +
                "${aimiFmt1(adjusted)} shadow=$shadowOnly corr=${aimiFmt2(gs.corrSqu)}"

        if (shadowOnly) {
            aapsLogger.debug(LTag.AIMI, "DYNISF_TRAJECTORY_SHADOW $logDetail")
            return DynIsfTrajectoryTuningOutcome(
                isfMgdlPerU = blendedMgdlPerU,
                appliedMultiplierToBlended = false,
                shadowOnly = true,
        ...
        aapsLogger.debug(LTag.AIMI, "DYNISF_TRAJECTORY_APPLY $logDetail")
        return DynIsfTrajectoryTuningOutcome(
    ...
    private fun noop(
        ...
        aapsLogger.debug(LTag.AIMI, "DYNISF_TRAJECTORY_SKIP reason=$reason shadow=$shadowOnly")
        return DynIsfTrajectoryTuningOutcome(
            isfMgdlPerU = blended,
            appliedMultiplierToBlended = false,
            shadowOnly = shadowOnly,
