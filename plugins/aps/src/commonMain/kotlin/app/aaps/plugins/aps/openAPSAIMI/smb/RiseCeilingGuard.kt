 ...
 *
 * Source: `origin/dev_OAPSAIMI` @ `9a234151c7` (file SHA `5741c83dfb`, unchanged on tip `acdc2b118e`
 * and on `f61474bb73`). Study wiring: commonMain `internal object` (pure, no instance state, one
 * tick consumer). No Metro `@Inject`. Sibling [MaxSmbLadder] is the same shape. KMP adaptation only:
 * [aimiFmt1] instead of `String.format(Locale.US)`. Clinical formulas, thresholds, and reason tokens
 * are unchanged.
 */
internal object RiseCeilingGuard {
    ...
    fun evaluate(atCeiling: Boolean, repeats: Int, deltaMgdl5m: Double?): Verdict {
        ...
        // Decimal point is a dot on purpose: the reason string is parsed from the exported data, and
        // a locale that writes a comma for the decimal point would break every reader of it.
        val numbers = "repeats=$repeats delta=${aimiFmt1(deltaMgdl5m)}"
