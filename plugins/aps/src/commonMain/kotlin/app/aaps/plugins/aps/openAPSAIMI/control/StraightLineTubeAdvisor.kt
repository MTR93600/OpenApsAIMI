...
class StraightLineTubeAdvisor @Inject constructor(
    ...
    fun advise(input: Input): Outcome {
        ...
        if (!anyFeasible) {
            ...
            val msg =
                "VETO hypoTube minPred=${aimiFmt0(minPred)} floor=${aimiFmt0(hypoFloor)} κ=${aimiFmt1(kappa)} maxSmb=${aimiFmt2(smbMax)} basal×${aimiFmt2(bScale)}"
            aapsLogger.warn(LTag.AIMI, "[$TAG] $msg")
            return Outcome(
                smbCapScale = 0.0,
        ...
        val reason =
            "s=${aimiFmt2(bestS)} basal×${aimiFmt2(bScale)} J=${aimiFmt1(bestCost)} " +
                "minPred=${aimiFmt0(minPred)}→${aimiFmt0(minAfterBest)} ev=${aimiFmt0(input.eventualBgMgdl)} " +
                "κ=${aimiFmt1(kappa)} COB=${aimiFmt1(input.cobG)} IOB=${aimiFmt2(input.iobU)}"
        aapsLogger.info(LTag.AIMI, "[$TAG] $reason")
        return Outcome(
            smbCapScale = bestS,
            basalCapScale = bScale,
            feasible = true,
