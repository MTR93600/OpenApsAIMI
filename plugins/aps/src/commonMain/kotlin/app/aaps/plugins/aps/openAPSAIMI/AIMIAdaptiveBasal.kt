    ...
    companion object {
        ...
        fun pureSuggest(input: Input): Decision {
            ...
            if (input.lastTempIsZero && input.zeroSinceMin >= settings.zeroResumeMin) {
                ...
                val r = "micro-resume after ${input.zeroSinceMin}m @0U/h → ${aimiFmt2(rate)}U/h × ${dur}m"
                return Decision(rate, dur, r)
            }
            ...
            if (input.profileBasal > 0 && input.predictedBg > 75.0 && input.auditorConfidence > 0.60 && input.bg < 115.0) {
                 if (input.delta > -6.0) {
                     val floorRate = max(0.1, input.profileBasal * 0.40)
                     if (input.lastTempIsZero || (input.minutesSinceLastChange > 5 && input.delta < 0)) {
                         val r = "LLM-SoftFloor (Pred=${aimiFmt0(input.predictedBg)}, Conf=${aimiFmt0(input.auditorConfidence * 100)}%) → ${aimiFmt2(floorRate)}U/h"
                         return Decision(floorRate, 30, r)
                     }
                 }
            }
            ...
            if (highAndFlat) {
                ...
                val r = "plateau kicker (BG=${aimiFmt0(input.bg)}, Δ≈0, R2=${aimiFmt2(input.r2)}) → ${aimiFmt2(target)}U/h × ${dur}m"
                return Decision(target, dur, r)
            }
            ...
            if (glued && input.bg > settings.highBg && input.delta < settings.deltaPosRelease) {
                ...
                val dur = 10
                val r = "anti-stall bias (+${(settings.antiStallBias*100).toInt()}%) because R2=${aimiFmt2(input.r2)} & Δ≈0"
                return Decision(rate, dur, r)
            }
