...
class AdaptivePkPdEstimator(
    ...
    private var state: PkPdParams = initial
    private var lastUpdateEpochMin: Long = 0
    private var acceptedUpdateCount = 0L
    private var status = AdaptivePkPdStatusSnapshot(
        params = initial,
    ...
    fun params(): PkPdParams = lock.withLock { state }
    ...
    fun update(
        ...
        lock.withLock {
            ...
            val acceptedCount = acceptedUpdateCount
            val updateAt = epochMin * 60_000L
            status = AdaptivePkPdStatusSnapshot(
                params = newParams,
    ...
    fun statusSnapshot(): AdaptivePkPdStatusSnapshot = lock.withLock { status.copy() }
    ...
    fun iobResidualAt(minFromDose: Double): Double {
        val params = lock.withLock { state }
    ...
    fun actionAt(minFromDose: Double): Double =
        kernel.actionAt(minFromDose, lock.withLock { state }).coerceAtLeast(0.0)
