...
data class AuditorProfileContext(
     ...
     */
    internal fun toPromptJson(): JsonObj {
    ...
    private fun buildJson(withRunningBasal: Boolean, withCommandIsf: Boolean): JsonObj =
        JsonObj().apply {
            ...
            put("meal_mode", mealModeName ?: AimiJson.NULL)
            put("meal_certainty", mealCertaintyLevel ?: AimiJson.NULL)
    ...
    private fun numberOrNull(value: Double?, decimals: Int): Any =
        value?.takeIf { it.isFinite() }?.let { roundTo(it, decimals) } ?: AimiJson.NULL
