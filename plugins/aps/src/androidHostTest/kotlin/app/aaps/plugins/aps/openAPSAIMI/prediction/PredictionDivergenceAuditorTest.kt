...
class PredictionDivergenceAuditorTest {
    ...
    fun `json export carries the audit fields`() {
        ...
        assertEquals(145.0, json.optDoubleCompat("bg_mgdl", Double.NaN), 1e-9)
        assertEquals(95.0, json.optDoubleCompat("pkpd_eventual_mgdl", Double.NaN), 1e-9)
        assertEquals(160.0, json.optDoubleCompat("scenario_best_mgdl", Double.NaN), 1e-9)
        assertEquals(65.0, json.optDoubleCompat("divergence_mgdl", Double.NaN), 1e-9)
        assertEquals("CORTISOL_MORNING_RISE", json.optStringCompat("physio_phase"))
        assertTrue(json["meal_phase"] is JsonNull)
        assertTrue(json.optBooleanCompat("pkpd_triggers_low_clamp"))
        assertFalse(json.optBooleanCompat("scenario_triggers_low_clamp"))
        assertTrue(json.optBooleanCompat("low_clamp_disagreement"))
    }
