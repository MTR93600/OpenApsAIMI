...
class PostHypoDeliveryAuthorityTest {
    ...
    fun export_namesTheDecliningConditionAndTheCapEffect() {
        ...
        assertFalse(declinedJson.optBooleanCompat("active"))
        assertEquals(PostHypoDeliveryAuthority.REASON_MEAL_EVIDENCE, declinedJson.optStringCompat("reason_tag"))
        assertTrue(declinedJson["max_smb_u"] is JsonNull)
        assertEquals(0.77, declinedJson.optDoubleCompat("smb_before_cap_u", Double.NaN), 0.001)
        assertEquals(0.77, declinedJson.optDoubleCompat("smb_after_cap_u", Double.NaN), 0.001)
        ...
        assertTrue(appliedJson.optBooleanCompat("active"))
        assertEquals(0.0, appliedJson.optDoubleCompat("max_smb_u", Double.NaN), 0.001)
        assertEquals(0.77, appliedJson.optDoubleCompat("smb_before_cap_u", Double.NaN), 0.001)
        assertEquals(0.0, appliedJson.optDoubleCompat("smb_after_cap_u", Double.NaN), 0.001)
    }
