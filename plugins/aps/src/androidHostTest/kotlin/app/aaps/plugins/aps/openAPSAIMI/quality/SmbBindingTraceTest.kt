...
class SmbBindingTraceTest {
    ...
    fun toJsonObject_exports_binding_fields_and_full_stage_chain() {
        ...
        val stages = json.optJsonArrayCompat("stages")!!

        assertThat(json.optStringCompat("binding_stage")).isEqualTo("PATTERN_CAP")
        assertThat(json.optDoubleCompat("final_u", Double.NaN)).isEqualTo(1.2)
        assertThat(json.optDoubleCompat("max_smb_high_bg_u", Double.NaN)).isEqualTo(2.2)
        assertThat(stages.size).isEqualTo(2)
        assertThat((stages[1] as JsonObject).optBooleanCompat("reduced")).isTrue()
        assertThat(json.optStringCompat("origin_owner")).isEqualTo("AutodriveV3")
        assertThat(json.optStringCompat("final_owner")).isEqualTo("GlobalAIMI")
        assertThat(json.optStringCompat("stage_structure")).isEqualTo("ORDERED_EVENTS_NOT_STRICTLY_CONTIGUOUS")
        assertThat((stages[0] as JsonObject).hasCompat("reference_u")).isTrue()
        assertThat((stages[0] as JsonObject).hasCompat("limit_u")).isFalse()
        assertThat((stages[0] as JsonObject).optStringCompat("kind")).isEqualTo("FLOOR")
    }
