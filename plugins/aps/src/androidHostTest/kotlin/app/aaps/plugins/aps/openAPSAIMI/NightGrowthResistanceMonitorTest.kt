...
class NightGrowthResistanceMonitorTest {
    ...
    private val monitor = DefaultNightGrowthResistanceMonitor(TimeZone.UTC)
    private val config = NGRConfig(
        enabled = true,
        pediatricAgeYears = 10,
        nightStart = LocalTime(22, 0),
        nightEnd = LocalTime(7, 0),
        minRiseSlope = 3.0,
        minDurationMin = 15,
    ...
    fun `mode retains an immutable copy of latest result`() {
        val mode = NightGrowthResistanceMode(DefaultNightGrowthResistanceMonitor(TimeZone.UTC))
