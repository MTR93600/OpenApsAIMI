...
class aimiNeuralNetworkTest {
    ...
    fun `a weight file without the schema version is refused`(@TempDir dir: File) {
        val storage = storageFor(dir)
        val legacy = storage.file("legacy_weights.json")
        storage.writeText(
            legacy,
            """
        ...
        assertNull(AimiNeuralNetwork.loadFromFile(storage, legacy), "Old schema files must not be loaded")
    }
    ...
    fun `save and load keeps the model and its normalization`(@TempDir dir: File) {
        ...
        val storage = storageFor(dir)
        val file = storage.file("weights.json")
        nn.saveToFile(storage, file)
        val loaded = AimiNeuralNetwork.loadFromFile(storage, file)
    ...
    /** [AimiStorage] backed by [dir], the same pattern `RecursiveBeliefExportReaderTest` uses. */
