...
class AimiRetentionWorker @AssistedInject constructor(
     ...
     *
     * **It must also be registered in `AppWorkersGraph` with a `@WorkerKey`.** An unregistered worker
     * makes `MetroWorkerFactory` answer null, WorkManager then falls back to reflection, and reflection
     * can only call a `(Context, WorkerParameters)` constructor - which this one does not have. The
     * worker would fail to instantiate on every run and never log a thing.
     */
    @AssistedFactory
