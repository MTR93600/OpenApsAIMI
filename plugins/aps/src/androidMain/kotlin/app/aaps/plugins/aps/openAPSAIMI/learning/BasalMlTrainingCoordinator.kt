...
class BasalMlTrainingCoordinator @Inject constructor(
    ...
    private val trainScope = CoroutineScope(aapsIoDispatcher + SupervisorJob())
    ...
    fun maybeTrainAsync() {
        trainScope.launch {
            ...
            try {
                ...
                log.error(LTag.AIMI, "$TAG: maybeTrainAsync failed", e)
            }
        }
    }
        ...
        return try {
            ...
            when {
                ...
                published -> {
                    ...
                    log.info(LTag.AIMI, "$TAG: training complete — models reloaded ($totalRows CSV rows)")
                    ...
                    markAttemptCompleted(now, totalRows)
                    log.debug(LTag.AIMI, "$TAG: candidate did not beat the incumbent — attempt recorded")
                    TrainingOutcome.SKIPPED
                }
            }
        } catch (e: Exception) {
            recordFailure()
            log.error(LTag.AIMI, "$TAG: training exception", e)
            TrainingOutcome.FAILED_RETRY
        }
    }
    ...
    private fun recordFailure() {
        if (circuitBreaker.recordFailure()) {
            log.warn(LTag.AIMI, "$TAG: circuit breaker OPEN for 6h after ${TrainingCircuitBreaker.DEFAULT_MAX_FAILURES} failures")
        }
    }
