...
class ComparisonCsvParser {
        ...
        val reason = when (preferredAlgorithm) {
            "AIMI" -> {
                if (aggressivenessRatio > 2.0) {
                    "SMB ${aimiFmt1(aggressivenessRatio)}x plus agressif avec variabilité ${safety.variabilityLabel.lowercase()}"
                } else {
            ...
            else -> "Les deux algorithmes montrent des performances similaires (${aimiFmt1(stats.agreementRate)}% d'accord)"
        }
    ...
    fun generateLlmSummary(
        ...
        sb.append("- Agreement Rate: ${aimiFmt1(stats.agreementRate)}%\n")
        sb.append("- AIMI More Aggressive: ${aimiFmt1(stats.aimiWinRate)}%\n")
        sb.append("- SMB More Aggressive: ${aimiFmt1(stats.smbWinRate)}%\n")
        sb.append("- Total Insulin Difference: ${aimiFmt2(impact.cumulativeDiff)} U (AIMI - SMB)\n")
        
        sb.append("\n## 2. Safety Analysis\n")
        sb.append("- Variability Score: ${aimiFmt1(safety.variabilityScore)}/100 (${safety.variabilityLabel})\n")
        sb.append("- Estimated Hypo Risk: ${safety.estimatedHypoRisk}\n")
        sb.append("- Safety Note: ${recommendation.safetyNote}\n")
