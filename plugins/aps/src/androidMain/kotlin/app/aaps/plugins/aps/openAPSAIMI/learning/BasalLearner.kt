...
class BasalLearner @Inject constructor(
    ...
    fun onPersistentHyper() {
        ...
        if (now - lastHypoDetectedAt < POST_HYPO_EXCLUSION_MS) {
            log.info(LTag.AIMI, "BasalLearner: Persistent hyper ignored (post-hypo rebound window)")
            return
        }
        log.info(LTag.AIMI, "BasalLearner: Persistent hyper detected, increasing short-term multiplier")
    ...
    private fun updateShortTerm(now: Long) {
        ...
        log.debug(LTag.AIMI, "BasalLearner: Short-term update. WeightedError=${aimiFmt2(weightedError)}, " +
            "Adjustment=$adjustment, NewMultiplier=${fmt3(shortTermMultiplier)}")
    }
    ...
    private fun updateMediumTerm(now: Long) {
        ...
        log.debug(LTag.AIMI, "BasalLearner: Medium-term update. AvgBG=${aimiFmt0(avgBg)}, " +
            "WeightedError=${aimiFmt2(weightedError)}, NewMultiplier=${fmt3(mediumTermMultiplier)}")
    }
    ...
    private fun updateLongTerm(tdd7Days: Double, tdd30Days: Double) {
        ...
        log.info(LTag.AIMI, "BasalLearner: Long-term update. TDD7/30=${aimiFmt2(tdd7Days/max(1.0, tdd30Days))}, " +
            "AvgFastingBG=${aimiFmt0(avgFastingBg)}, FastingSlope=${aimiFmt2(fastingScore)}, " +
            "NewMultiplier=${fmt3(longTermMultiplier)}")
    }
    ...
    private fun save() {
        val json = buildJsonObject {
            put("shortTermMultiplier", shortTermMultiplier)
            put("mediumTermMultiplier", mediumTermMultiplier)
            put("longTermMultiplier", longTermMultiplier)
            put("lastShortUpdate", lastShortUpdate)
