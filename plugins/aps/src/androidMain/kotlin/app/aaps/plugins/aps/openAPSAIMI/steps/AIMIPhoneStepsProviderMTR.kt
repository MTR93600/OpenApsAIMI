...
class AIMIPhoneStepsProviderMTR @Inject constructor(
    ...
    override fun getStepsDelta(windowMinutes: Int, now: Instant): Int {
        return try {
            ...
            if (steps > 0) {
                aapsLogger.debug(LTag.AIMI, "[$SOURCE_NAME] Retrieved $steps steps for {$windowMinutes}min")
            }

            steps
        } catch (e: Exception) {
            aapsLogger.error(LTag.AIMI, "[$SOURCE_NAME] Error fetching steps for {$windowMinutes}min", e)
            0
        }
    }
    ...
    override fun getLastUpdateMillis(): Long {
        ...
        return try {
            ...
            if (has5MinData) aimiWallClockMs() else 0L
        } catch (e: Exception) {
            0L
        }
    }
    ...
    override fun isAvailable(): Boolean {
        ...
        return try {
            ...
            aapsLogger.error(LTag.AIMI, "[$SOURCE_NAME] Availability check failed", e)
            false
        }
    }
