...
class AIMIPhysioContextStoreMTR @Inject constructor(
    ...
    private fun saveToDisk(
        ...
        try {
            ...
            aapsLogger.info(LTag.AIMI, "[$TAG] 💾 PhysioStore: writing to ${storageFile.absolutePath}")
        ...
        aapsLogger.debug(LTag.AIMI, "[$TAG] Saved to ${storageFile.absolutePath} (${storageFile.length()} bytes)")
    }
    ...
    private fun restoreFromDisk() {
        ...
        try {
            val json = Json.parseToJsonElement(storageFile.readText()).jsonObject
