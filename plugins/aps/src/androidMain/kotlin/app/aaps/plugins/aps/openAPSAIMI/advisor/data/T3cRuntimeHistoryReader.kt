...
internal object T3cRuntimeHistoryReader {
  ...
  /** Capped for Advisor launch on 256MB heaps; 24h of ticks fits well below this. */
    ...
    fun summarizeLast24Hours(
        ...
        val statusCounts = T3cRuntimeTickStatus.entries.associateWith { status ->
            records.count { it.status == status }
        }
        val dominantStatus = T3cRuntimeTickStatus.entries.maxByOrNull { statusCounts[it] ?: 0 }
    ...
    private fun parseTick(root: JsonObject): T3cRuntimeTickRecord? {
        ...
        val ownership = adjustments.optJsonObjectCompat("t3c_runtime_ownership")
        val resolution = recursiveBelief?.optJsonObjectCompat("resolution")
        val t3c = resolution?.optJsonObjectCompat("t3c_basal_first")
        if (ownership == null && t3c == null) return null

        val authorityApplied = recursiveBelief?.optBooleanCompat("authority_applied") ?: false
        val shadowOnly = recursiveBelief?.optBooleanOrDefault("shadow_only", true) ?: true
        val active = t3c?.optBooleanCompat("active") ?: false
        val eligible = t3c?.optBooleanCompat("eligible") ?: false
        val mealConflict = t3c?.optBooleanCompat("meal_conflict") ?: false
        val postHypoBlock = t3c?.optBooleanCompat("post_hypo_block") ?: false
        val exerciseBlock = t3c?.optBooleanCompat("exercise_block") ?: false
        val hardSafetyBlock = t3c?.optBooleanCompat("hard_safety_block") ?: false
        val nativeOwnerActive = ownership?.optBooleanCompat("native_owner_active") ?: false
        val legacyFallbackAllowed = ownership?.optBooleanCompat("legacy_fallback_allowed") ?: false
        ...
        return T3cRuntimeTickRecord(
            ...
            blocker = blocker,
            selectedForProduction = t3c?.optBooleanCompat("selected_for_production") ?: false,
            historicalBypassNeutralized = t3c?.optBooleanCompat("historical_bypass_neutralized") ?: false,
        )
    }
    ...
    private fun JsonObject.optBooleanOrDefault(key: String, defaultValue: Boolean): Boolean {
        if (!hasCompat(key) || this[key] is JsonNull) return defaultValue
        return optBooleanCompat(key)
    }
    ...
    private fun JsonObject?.optStringOrDefault(
        ...
        this?.optStringCompat(key)?.takeIf { it.isNotBlank() } ?: defaultValue

    private fun JsonObject.optStringOrNull(key: String): String? =
        if (jsonIsNull(key)) null else optStringCompat(key).takeIf { it.isNotBlank() }

    private fun JsonObject.optDoubleOrNull(key: String): Double? =
        if (jsonIsNull(key)) null else optDoubleCompat(key, Double.NaN)
