...
class AutodriveNeuralTrainer @Inject constructor(
    ...
    private val prettyWeightsJson = Json {
        prettyPrint = true
        prettyPrintIndent = "    "
    }
    ...
    private fun loadIncumbent(): Model? = try {
        ...
        if (!file.exists() || file.length() == 0L) {
            ...
            Model(
                doubleArrayOf(
                    json.optDoubleCompat("weight_hr", 0.0),
                    json.optDoubleCompat("weight_inflammation", 0.0),
                    json.optDoubleCompat("weight_hormonal", 0.0),
                    json.optDoubleCompat("weight_engaged", 0.0),
                ),
                bias,
            )
        }
    } catch (e: Exception) {
        aapsLogger.warn(LTag.AIMI, "NeuralTrainer: installed weights unreadable, treated as absent — ${e.message}")
        null
    }
    ...
    private fun saveLearnedWeights(model: Model, positiveRate: Double, report: TrainingReport): Boolean {
        return try {
            ...
            val json = buildJsonObject {
                put("bias", calibratedBias)
                put("bias_balanced", model.bias)
                put("weight_hr", model.weights[0])
                put("weight_inflammation", model.weights[1])
                put("weight_hormonal", model.weights[2])
                put("weight_engaged", model.weights[3])
                put("timestamp", aimiWallClockMs())
                // Liveness: "the model says safe" and "there is no model" must not look the same.
                put("rows", report.rows)
                put("positives", report.positives)
                put("positive_rate", positiveRate)
                put("holdout_rows", report.holdoutRows)
                put("holdout_positives", report.holdoutPositives)
                put("holdout_balanced_log_loss", report.candidateLoss)
                put("holdout_base_rate_log_loss", report.baseRateLoss)
            ...
            aapsLogger.error(LTag.AIMI, "NeuralTrainer Error saving weights: ${e.message}")
            false
        }
    }
