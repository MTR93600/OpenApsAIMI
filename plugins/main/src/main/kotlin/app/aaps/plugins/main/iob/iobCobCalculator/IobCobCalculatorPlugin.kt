...
class IobCobCalculatorPlugin @Inject constructor(
    ...
    override fun calculateFromTreatmentsAndTemps(toTime: Long, profile: Profile): IobTotal {
        ...
        if (tb.timestamp < time) {
            val calc = tb.iobCalc(time, profile, activePlugin.activeInsulin)
    ...
    private fun calculateIobFromBolusToTime(toTime: Long): IobTotal {
        ...
        boluses.forEach { bs ->
            if (bs.isValid && bs.timestamp < toTime) {
                val tIOB = bs.iobCalc(activePlugin, toTime, dia)
                total.iob += tIOB.iobContrib
                total.activity += tIOB.activityContrib

                if (bs.amount > 0 && bs.timestamp > total.lastBolusTime) {
                    total.lastBolusTime = bs.timestamp
                }

                if (bs.type != BS.Type.SMB) {
                    // instead of dividing the DIA that only worked on the bilinear curves,
                    // multiply the time the treatment is seen active.
                    val timeSinceTreatment = toTime - bs.timestamp
                    val snoozeTime = bs.timestamp + (timeSinceTreatment * divisor).toLong()
                    val bIOB = bs.iobCalc(activePlugin, snoozeTime, dia)
                    total.bolussnooze += bIOB.iobContrib
                }
            }
        }

        val extendedBolusIob = calculateIobToTimeFromExtendedBoluses(toTime)
        total.plus(extendedBolusIob)
    ...
    private fun calculateIobToTimeFromExtendedBoluses(toTime: Long): IobTotal {
        ...
        if (!pumpInterface.isFakingTempsByExtendedBoluses) {
            ...
            for (pos in extendedBoluses.indices) {
                val eb = extendedBoluses[pos]

                if (eb.timestamp > toTime) {
                    continue
                }

                if (eb.end > now) {
                    val newDuration = now - eb.timestamp
                    eb.amount *= newDuration.toDouble() / eb.duration
                    eb.duration = newDuration
                }

                val profile = profileFunction.getProfile(eb.timestamp) ?: return total
                val calc = eb.iobCalc(toTime, profile, activePlugin.activeInsulin)
                total.plus(calc)
            }
        }
