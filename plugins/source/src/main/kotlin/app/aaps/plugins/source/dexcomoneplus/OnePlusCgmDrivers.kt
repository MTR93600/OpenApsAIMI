...
/**
 * Driver selection. The Real driver is what users get: the preference behind it defaults to true,
 * and outside engineering mode a preference always reads its default. The Stub stays for tests and
 * for an engineer who wants the plugin without a radio.
 *
 * Use [select] (not a bare flag flip) so watchers stay on the active instance when that preference
 * changes.
 */
object OnePlusCgmDrivers {
    ...
    /** Production path — Real unless an engineer turned the Real driver off. */
