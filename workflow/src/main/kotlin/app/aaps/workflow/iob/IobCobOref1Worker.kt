        ...
        try {
            ...
            for (i in bucketedData.size - 4 downTo 0) {
                rxBus.send(EventIobCalculationProgress(
                    CalculationWorkflow.ProgressData.IOB_COB_OREF,
                    100 - (100.0 * i / bucketedData.size).toInt(), data.cause)
                )
                ...
                if (bgTime > ads.roundUpTime(dateUtil.now())) {
                    continue
                }
                ...
                if (i < bucketedData.size - 16) { // we need 1h of data to calculate minDeviationSlope
                    ...
                    if (hourAgoData != null) {
                        ...
                        aapsLogger.debug(LTag.AUTOSENS) {
                            ">>>>> bucketed_data.size()=" + bucketedData.size + " i=" + i + " hourAgoData=" + hourAgoData.toString()
                        }
...
//                        try {
                ...
                if (previous != null && previous.cob > 0) {
                    ...
                    if (ci != deviation) {
                        autosensData.failOverToMinAbsorptionRate = true
                    }
