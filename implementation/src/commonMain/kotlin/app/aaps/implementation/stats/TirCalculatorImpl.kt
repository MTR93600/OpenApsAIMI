...
class TirCalculatorImpl @Inject constructor(
    ...
    /**
     * ⚠️ ASYNC IMPACT: freeze AIMI called these from a sync tick. Persistence is suspend, so this
     * blocks like the freeze `runBlocking` wrappers.
     */
    override fun calculateHour(lowMgdl: Double, highMgdl: Double): LongSparseArray<TIR> {
        ...
        val bgReadings = runBlocking { persistenceLayer.getBgReadingsDataFromTimeToTime(startTime, endTime, true) }
        val result = LongSparseArray<TIR>()
        for (bg in bgReadings) {
